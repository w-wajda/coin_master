import uuid
from unittest.mock import MagicMock

import pytest

from app.application.queries.tags.get_tag import GetTagQuery
from app.domain.exceptions import HTTPException
from tests.units.application._helpers import make_repository


@pytest.fixture
def user_repository():
    return make_repository()


@pytest.fixture
def tag_repository():
    return make_repository()


@pytest.fixture
def query(user_repository, tag_repository):
    return GetTagQuery(user_repository=user_repository, tag_repository=tag_repository)


async def test_get_tag_success(query, user_repository, tag_repository):
    user_repository.get.return_value = object()
    tag = MagicMock()
    tag_repository.get_by.return_value = tag

    result = await query(user_id=1, uuid=uuid.uuid4())

    assert result is tag


async def test_get_tag_user_not_found(query, user_repository, tag_repository):
    user_repository.get.return_value = None

    with pytest.raises(HTTPException) as exc_info:
        await query(user_id=1, uuid=uuid.uuid4())

    assert exc_info.value.status_code == 404
    tag_repository.get_by.assert_not_awaited()


async def test_get_tag_not_found(query, user_repository, tag_repository):
    user_repository.get.return_value = object()
    tag_repository.get_by.return_value = None

    with pytest.raises(HTTPException) as exc_info:
        await query(user_id=1, uuid=uuid.uuid4())

    assert exc_info.value.status_code == 404
